...
export default function Deposit() {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      const res = await deposit(id.trim(), parseFloat(amount))
      setResult(res)
    } catch (e) {
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold mb-1">Dépôt</h1>
      <p className="text-sm text-stone-500 mb-6">Crédite un montant sur un compte existant.</p>

      <div className="card">
        <RouteTag method="POST" path="/api/accounts/{id}/deposit" />
