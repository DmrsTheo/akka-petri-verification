package com.music.payment

import akka.actor.typed.ActorSystem
import akka.actor.typed.scaladsl.Behaviors
import akka.actor.typed.scaladsl.AskPattern._
import akka.util.Timeout
import com.music.payment.actors.BankSupervisor
import com.music.payment.messages.PaymentMessages._
import com.music.simulation.SimulationRunner

import scala.concurrent.{Await, ExecutionContext, Future}
import scala.concurrent.duration._
import scala.io.StdIn

/**
 * Point d'entrée principal de l'application.
 *
 * Propose deux modes :
 * 1. Mode automatique : exécute un scénario prédéfini + analyse Petri Net
 * 2. Mode interactif : CLI manuelle pour interagir avec le système bancaire
 */
object Main extends App {

  println()
  println("=" * 70)
  println("  SYSTÈME DE PAIEMENT DISTRIBUÉ — AKKA + VÉRIFICATION PETRI NET")
  println("=" * 70)
  println()
  println("  Choisissez un mode :")
  println("    1. Mode automatique (scénario prédéfini + analyse Petri Net)")
  println("    2. Mode interactif (commandes manuelles)")
  println()
  print("  Votre choix [1/2] : ")

  val choice = StdIn.readLine().trim

  choice match {
    case "2" => runInteractiveMode()
    case _   => runAutomaticMode()
  }

  // ===== Mode interactif =====
  def runInteractiveMode(): Unit = {
    val system = ActorSystem(BankSupervisor(), "payment-system")

    try {
      InteractiveCLI.run(system)
    } finally {
      system.terminate()
      Await.result(system.whenTerminated, 10.seconds)
      println("  Système arrêté. Au revoir !")
    }
  }

  // ===== Mode automatique (comportement original) =====
  def runAutomaticMode(): Unit = {

    println("━" * 50)
    println("  PARTIE 1: SIMULATION AKKA")
    println("━" * 50)
    println()

    implicit val timeout: Timeout = 5.seconds

    val system = ActorSystem(BankSupervisor(), "payment-system")
    implicit val ec: ExecutionContext = system.executionContext
    implicit val scheduler: akka.actor.typed.Scheduler = system.scheduler

    try {
      // Créer des comptes
      println("  → Création des comptes...")
      val create1 = system.ask[OperationResult](ref => CreateAccount("ACC-001", "Alice", 1000.0, ref))
      val create2 = system.ask[OperationResult](ref => CreateAccount("ACC-002", "Bob", 500.0, ref))
      val create3 = system.ask[OperationResult](ref => CreateAccount("ACC-003", "Charlie", 250.0, ref))

      val results = Await.result(Future.sequence(List(create1, create2, create3)), 10.seconds)
      results.foreach {
        case OperationSuccess(id, balance, msg) => println(s"    ✓ $msg (solde: $balance)")
        case OperationFailure(id, reason) => println(s"    ✗ Échec pour $id: $reason")
      }

      // Dépôts
      println("\n  → Opérations de dépôt...")
      val dep1 = system.ask[OperationResult](ref => PerformDeposit("ACC-001", 200.0, ref))
      val depResult = Await.result(dep1, 5.seconds)
      depResult match {
        case OperationSuccess(id, balance, _) => println(s"    ✓ Dépôt sur $id réussi. Nouveau solde: $balance")
        case OperationFailure(id, reason) => println(s"    ✗ Dépôt échoué sur $id: $reason")
      }

      // Retrait réussi
      println("\n  → Opérations de retrait...")
      val wd1 = system.ask[OperationResult](ref => PerformWithdraw("ACC-002", 100.0, ref))
      val wdResult = Await.result(wd1, 5.seconds)
      wdResult match {
        case OperationSuccess(id, balance, _) => println(s"    ✓ Retrait sur $id réussi. Nouveau solde: $balance")
        case OperationFailure(id, reason) => println(s"    ✗ Retrait refusé sur $id: $reason")
      }

      // Retrait refusé (fonds insuffisants)
      val wd2 = system.ask[OperationResult](ref => PerformWithdraw("ACC-003", 500.0, ref))
      val wdResult2 = Await.result(wd2, 5.seconds)
      wdResult2 match {
        case OperationSuccess(id, balance, _) => println(s"    ✓ Retrait sur $id réussi. Nouveau solde: $balance")
        case OperationFailure(id, reason) => println(s"    ✗ Retrait refusé sur $id: $reason")
      }

      // Transfert réussi
      println("\n  → Transferts entre comptes...")
      val tx1 = system.ask[TransactionResult](ref => PerformTransfer("ACC-001", "ACC-002", 300.0, ref))
      val txResult = Await.result(tx1, 5.seconds)
      txResult match {
        case TransferSuccess(from, to, amount, msg) => println(s"    ✓ $msg")
        case TransferFailure(from, to, amount, reason) => println(s"    ✗ Transfert échoué: $reason")
      }

      // Consulter les soldes
      println("\n  → Consultation des soldes...")
      for (accId <- List("ACC-001", "ACC-002", "ACC-003")) {
        val bal = system.ask[BalanceResponse](ref => QueryBalance(accId, ref))
        val balResult = Await.result(bal, 5.seconds)
        println(s"    Compte ${balResult.accountId}: solde = ${balResult.balance}")
      }

      // Transfert échoué (fonds insuffisants)
      println("\n  → Transfert avec fonds insuffisants...")
      val tx2 = system.ask[TransactionResult](ref => PerformTransfer("ACC-003", "ACC-001", 999.0, ref))
      val txResult2 = Await.result(tx2, 5.seconds)
      txResult2 match {
        case TransferSuccess(_, _, _, msg) => println(s"    ✓ $msg")
        case TransferFailure(_, _, _, reason) => println(s"    ✗ Transfert refusé: $reason")
      }

      println()
      println("  Simulation Akka terminée avec succès.")
      println()

    } finally {
      system.terminate()
      Await.result(system.whenTerminated, 10.seconds)
    }

    // === PARTIE 2 : Analyse formelle Petri Net ===
    SimulationRunner.runPetriNetAnalysis()
  }
}
